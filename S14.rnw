...
funguild <- funguild[!is.na(match(funguild$OTU_ID, rownames(taxRDP2))),]

match_interm <- match(funguild$OTU_ID, rownames(taxRDP2))
...
taxa_names(repset) <- unlist(strsplit(taxa_names(repset), split = " "))[seq(1, 2*length(taxa_names(repset)), by = 2)]
@
