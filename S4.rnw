...
\title{Appendix S4: results after UPARSE clustering discarding unique sequences (Usearch function sortbysize with argument -minsize 2) and keeping sequences classified by ITSx as Tracheophyta. Supplementary Materials of "Finding fungi in a needle stack: high alpha and low beta-diversity of foliar endophytic Ascomycetes revealed by metabarcoding in Corsican pine forests".}
...
\textit{Cyclaneusma minus} account for \Sexpr{round(max(rowSums(data.f3@otu_table))/sum(rowSums(data.f3@otu_table)), 4) * 100}\% of total sequences.


<<fig.cap="Number of sequences assigned to \textit{Cyclaneusma minus} across host age, elevation whitin tree and sites", fig.height=4, fig.width=4>>=
cycla <- as.vector(data.f3@otu_table["OTU_1",]/ colSums(data.f3@otu_table) * 100)

par(mfrow=c(1,2))
boxplot(cycla~data.f3@sam_data$Age, ylab="% Cyclaneusma minus")
boxplot(cycla~data.f3@sam_data$Elevation, ylab="% Cyclaneusma minus")
boxplot(cycla~data.f3@sam_data$Sites, ylab="% Cyclaneusma minus")
par(mfrow=c(1,1))
@

<<>>=
tapply(cycla, data.f3@sam_data$Age, mean)
tapply(cycla, data.f3@sam_data$Age, sd)

tapply(cycla, data.f3@sam_data$Elevation, mean)
tapply(cycla, data.f3@sam_data$Elevation, sd)

tapply(cycla, data.f3@sam_data$Sites, mean)
tapply(cycla, data.f3@sam_data$Sites, sd)

wilcox.test(cycla[data.f3@sam_data$Sites=="Asco"], cycla[data.f3@sam_data$Sites=="Bavella"])
wilcox.test(cycla[data.f3@sam_data$Sites=="Verghello"], cycla[data.f3@sam_data$Sites=="Bavella"])
wilcox.test(cycla[data.f3@sam_data$Sites=="Asco"], cycla[data.f3@sam_data$Sites=="Verghello"])
@
